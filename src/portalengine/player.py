...
class Player(Protocol):
	def __init__(self, camera: Camera) -> None: pass

	def update(self) -> None: pass

class StandardPlayer(Player):
	def __init__(self, camera: Camera, height_before_eyes: float=1.0) -> None:
		self.ctx = camera.ctx
		self.camera = camera
...
class FlyingPlayer(Player):
	def __init__(self, camera: Camera, height_before_eyes: float=1.0) -> None:
		...
		self.left_keys = [pygame.K_a]
		self.right_keys = [pygame.K_d]
		self.up_keys = [pygame.K_SPACE]
		self.down_keys = [pygame.K_LSHIFT]
		self.sprint_keys = [pygame.K_LCTRL]
	...
	def move(self):
		...
		if self.is_held(keys, self.up_keys):
			self.pos+=self.camera.ALWAYS_UP*velocity
			self.key_held = True
		if self.is_held(keys, self.down_keys):
			self.pos-=self.camera.ALWAYS_UP*velocity
			self.key_held = True
